...
const prompt = `
    あなたはタイ語教育の最高権威です。単語や慣用表現の「${word}」について、言語学的な視点と実戦的なコミュニケーションの両面から、完璧な解析を行ってください。
    ...
    - category: 意味に対応する品詞を必ず次のフォーマット（1. 〇〇 2. 〇〇 3.・・・）で記載する。categoryは改行しません。品詞の選び方は後述するcategoryの項目別ルールに従ってください。
    - example_th, example_phonetic, example_ja: それぞれの意味に対する例文を、必ず箇条書き（1. 〇〇\\n2. 〇〇\\n3.・・・）で複数生成し、改行（\\n）で繋いで一つの文字列として出力する。example_thの生成ルールは依頼内容に従ってください。
    - explanation: それぞれの意味の使い分けや文脈の判断方法を網羅する。他のルールは下記の「explanationの構成」に従ってください。
    ...
    - JSONの文字列値の中で改行する場合は、必ず「\\n」というエスケープ文字列を使用し、実際の改行（生改行）は絶対に出力しないでください。

    ## 発音表記（phonetic / example_phonetic）の厳守ルール
    - 音節ごとに必ず「-（ハイフン）」で繋いでください。
    - 各音節の母音の上に、5つの声調記号（á, à, â, ǎ, a ※平声は記号なし）を必ず付与してください。
    ...
    ## 項目別ルール
    - category: 品詞を次から一つ選べ。動詞, 名詞, 形容詞, 副詞, 接続詞, 前置詞, 助動詞, 代名詞, 文末詞, 慣用句, 構文。名詞の場合は類別詞も含めること。フォーマットは「名詞（類別詞: タイ語 発音）」
    - explanation: セクションごとに「改行2回（\\n\\n）」、項目内は「改行1回（\\n）」を使用。Markdown装飾（**等）は使用せず、記号（■、・、1.）で視認性を確保。
    - level: 次のCEFR基準で厳格に判定せよ。1(超基礎/A1), 2(日常/A2), 3(一般/B1-B2), 4(専門・抽象/C1), 5(稀・王室・古典/C2)

    ## explanationの構成（全7項目を網羅してください）
    ※もし単語が「慣用句」や「構文」である場合、解説のどこかに「[A] ก็ต่อเมื่อ [B]」のような、前後にどんな言葉が来るかを示す【公式】を必ず含めてください。
    ...
    4. ■【類義語・反対語】: 1単語1行ずつ書いてください。
       類義語: タイ単語 (発音): 意味
       反対語: タイ単語 (発音): 意味
    5. ■【重要チャンク・成句】: 「${word}」を核とした、そのまま暗記すべき頻出フレーズを3つ以上。1フレーズ1行ずつ書いてください。
    6. ■【例文の全単語解析】: 例文(example_th)を構成する全てのパーツについて、 タイ単語（発音）: 意味 の形式でリスト化。多義語で例文が複数ある場合は、「【例文1】」「【例文2】」と見出しをつけて全て解析してください。1単語1行ずつ書いてください。
    7. ■【読み方や見た目が似た語】:日本人が混同しやすい単語を挙げてください。1単語1行ずつ書いてください。
       タイ単語 (発音): 意味
    ## JSON Structure
    {
      "phonetic": "発音記号",
...
※回答のみをマークダウン形式で出力してください。挨拶や前置きは不要です。`;

  return callGeminiApi(prompt);
}

/**
 * ユーザーの「こういう時なんていう？」を翻訳・解説するAI教師
 * 既存の callGeminiApi を再利用して通信を行う
 * @param {string} userMessage - ユーザーがチャットに入力したテキスト
 */
function askTranslationTeacher(userMessage) {
  if (!userMessage) return null;

  // ユーザーから渡された「神プロンプト」＋「表出力の追加指示」
  const systemPrompt = `あなたは、日本語、英語、タイ語に堪能な多言語翻訳のエキスパートであり、言語教師でもあります。これから私（ユーザー）が入力する文章に対して、以下のルールに厳密に従って応答を生成してください。
指定フォーマット以外の文章は不要です

# 背景情報
私は日本人で、タイ人と会話をしています。そのため、翻訳は自然でありながら、言語学習の観点から非常に正確である必要があります。

# 実行ルール
私が文章を入力します。
入力文章を、自然な英語に翻訳してください。
さらに入力文章を、ニュアンスを汲み取った日本語に翻訳してください。
最後に入力文章を、自然なタイ語に翻訳してください。タイ語の翻訳については、以下の形式で詳細な解説を日本語で加えてください。私の入力文章がタイ語の場合はタイ語の翻訳の箇所には原文を記載してください。
タイ語は男性の口調とし、発音記号のルールは以下を守ってください。
## 発音表記の厳守ルール
- 音節ごとに必ず「-（ハイフン）」で繋いでください。
- 各音節の母音の上に、5つの声調記号（á, à, â, ǎ, a ※平声は記号なし）を必ず付与してください。
- 【重要】特殊母音記号「ɛ, ɔ, ɯ」は積極的に使用してください。
...
### 文法解説
ここに、文の構造（SVOなど）、単語の並び順、助詞や接続詞の役割などについて詳しく解説してください。

### フレーズカード作成
ここに、今回翻訳した「文章全体（言い回し）」を1つのフレーズとして表形式（タイ語、発音記号、日本語の意味）で出力してください。

### 単語帳作成
ここに、すべての単語を表形式（タイ語、発音記号、日本語）で抽出してください。シートの作成は不要です。`;

  // システムプロンプトとユーザーの入力をガッチャンコする
  const finalPrompt = `${systemPrompt}\n\nユーザーの入力文章：\n${userMessage}`;

  // ★ 既存の共通エンジン「callGeminiApi」に投げるだけ！
...
以下のルールを絶対にお守りください：
1. 出力は必ずMarkdown形式とし、視覚的にわかりやすくするため、箇条書きを積極的に使ってください。
2. 回答の中に出てくるタイ語には、必ず「日本語の意味」と「発音記号」を添えてください。
3. 学習者が理解しやすいよう、具体的な例文を必ず提示してください。
4. 【重要】解説の中で新しい「単語」や「フレーズ」を紹介する場合は、必ず以下の3列のMarkdownテーブルにまとめて出力してください。この表はシステムが単語カードとして抽出するために使われます。
   | タイ語 | 発音記号 | 日本語 |
   |---|---|---|
   | (例: ไป) | (例: pai) | (例: 行く) |
5. 単語カード用AIと統一するため、発音記号は以下のルールを厳守してください：
   - 音節ごとに必ず「-（ハイフン）」で繋ぐ。
