...
const prompt = `
    ...
    - ピアノ: 
        - 桐朋学園大学院大学の科目等履修生。シューベルト、バッハ、ショパン、シマノフスキ、ベートーヴェン、ブラームス、ドビュッシー等を愛好する高度なピアニスト。
        - 楽曲の構造分析、音色の追求に加え、音楽を通じた哲学的探求を重視。
    - 思想: ソクラテスやニーチェ、カント等の西洋哲学に関心が深く、存在や道徳、美学についての深い洞察を好む。
    - 趣味・生活: タイ旅行、美術館巡り。友人とは音楽・芸術・哲学についての知的な会話を楽しむ。
    
    ## 依頼内容
    単語「${word}」について、以下の制約を守って解析してください。
    例文(example_th)は、以下の4つのシーンから、単語の性質に最も合うものを一つ選んで作成してください。言語学習に利用するので、日常でも実践できる簡潔な文とし、個人名や専門用語はなるべく使わず、長すぎないようにしてください。以下は優先順です。
    1. 【日常会話】: 日常で使う必須フレーズ。
    ...
    3. ■【実戦的使い分け】: 日本人が混同しやすい類似表現との決定的な違い。使用すべきではない文脈や、丁寧度のグラデーション（丁寧/普通/粗野）。専門的な文脈における精緻な用法。
    4. ■【類義語・反対語】: 
       類義語: 単語 (読み): 意味
       反対語: 単語 (読み): 意味
