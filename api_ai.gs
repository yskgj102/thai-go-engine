...
const prompt = `
    ...
    - 職業: PFU株式会社のITエンジニア。製造業のシステム開発に従事。
    - ピアノ: 
    ...
    - 趣味・生活: タイ旅行（上質な文化交流）、美術館巡り。友人とは音楽・芸術・哲学についての知的な会話を楽しむ。
    
    ## 依頼内容
    単語「${word}」について、以下の制約を守って解析してください。
    例文(example_th)は、以下の4つのシーンから、単語の性質に最も合うものを一つ選んで作成してください。言語学習に利用するので、日常でも実践できる簡潔な文とし、専門用語はなるべく使わず、長すぎないようにしてください。
    1. 【日常会話】: 日常で使うフレーズなど、金沢の美しい街並み、四季、友人との芸術に関する対話。
