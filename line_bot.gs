...
// 🌟 AIへの指示：初心者向けに難易度を下げたやさしいクイズ（シャッフル対策＆3択固定版）
  const prompt = `あなたはプロのタイ語教師です。
  タイ語初心者の学習者に向けて、基礎的でわかりやすい「3択クイズ」を作成してください。
  お題は日本語の「${meaning_ja}」（タイ語の正解: ${word_th}）です。

  【🚨重要・厳守ルール】
  1. 問題文やお題にはタイ語（${word_th}）を絶対に書かず、「『${meaning_ja}』と言いたい時、どのタイ語を使いますか？」といったシンプルで優しい形式にすること。
  2. 選択肢のテキスト（text）内に「(正解)」「〇」などのヒントは一切含めないこと。純粋な「タイ語と発音記号のみ」を出力してください。
  3. 🌟【初心者向けルール】不正解のダミーは、マニアックな声調違いや意地悪な引っかけを絶対に避けてください。「意味が全く違う基礎単語」や「明らかに文脈に合わない簡単な単語」を設定し、初心者が消去法で正解を選びやすいようにしてください。
  4. 【絶対厳守】選択肢（choices）は「必ず3つのみ（正解1つ、不正解2つ）」出力してください。絶対に4つ以上作らないでください。
  5. 🌟【解説の書き方・超重要】LINE上で選択肢をランダムにシャッフルするため、解説（explanation）の中で「選択肢1」「A」「B」「C」のような【順番や記号による言及】は絶対に禁止です。必ず「『〇〇（タイ語）』の場合は〜」のように、【具体的な選択肢のタイ語】をそのまま引用して解説してください。また、初心者が自信を持てるよう、優しく褒めるトーンにしてください。

  【出力形式】（以下のJSON形式のみを出力すること）
  {
    "question": "初心者にわかりやすい優しい問題文",
    "choices": [
