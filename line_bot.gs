...
// 🌟 AIへの指示：初心者向け＆実践的な文脈の穴埋めクイズ（シャッフル対策・3択固定・発音記号併記版）
  const prompt = `あなたはプロのタイ語教師です。
  タイ語初心者の学習者に向けて、基礎的でわかりやすい「穴埋め形式の3択クイズ」を作成してください。
  お題となる正解の単語は「${word_th}」（意味: ${meaning_ja}）です。

  【🚨重要・厳守ルール】
  1. 問題文（question）は、レストランでの注文、買い物、ホテルでの会話など、実践的な日常会話の文脈にしてください。
  2. 問題文はタイ語で作成し、正解の単語「${word_th}」が入るべき場所を「（  ）」としてください。
  3. 🌟【初心者向けルール】問題文のタイ語のすぐ後に、必ず【発音記号】を併記し、その後に日本語訳を添えてください。
     例：「ขอ（  ）หน่อยครับ [khɔ̌ɔ ( ) nɔ̀y khráp] (お会計をお願いします)」
  4. 🌟【発音記号の厳守ルール】※問題文・選択肢ともに適用
     - 音節ごとに必ず「-（ハイフン）」で繋ぐこと。
     - 各音節の母音の上に、5つの声調記号（á, à, â, ǎ, a ※平声は記号なし）を必ず付与すること。
     - 特殊母音記号「ɛ, ɔ, ɯ」を積極的に使用すること。
     - アルファベット（ae, oo, uue）による代用ではなく、これら特殊記号の上に声調記号を乗せた表記を徹底すること。
     - 表記例 : thɔ́ɔng-fáa, sà-wàt-dii, khɔ̀ɔp-khun, phûut, mɯɯ, kɛ̂ɛo, khəəi
  5. 🌟【初心者向けダミー】不正解のダミーは、文脈に全く合わない基礎単語を設定し、初心者が文脈や日本語訳から推測して消去法で正解を選びやすいようにしてください。
  6. 【絶対厳守】選択肢（choices）は「必ず3つのみ（正解1つ、不正解2つ）」出力してください。絶対に4つ以上作らないでください。
  7. 🌟【解説の書き方】LINE上で選択肢をランダムにシャッフルするため、解説（explanation）の中で「選択肢1」「A」「B」「C」のような【順番や記号による言及】は絶対に禁止です。必ず「『〇〇（タイ語）』の場合は〜」のように、【具体的な選択肢のタイ語】をそのまま引用して解説してください。また、初心者が自信を持てるよう、優しく褒めるトーンにしてください。

  【出力形式】（以下のJSON形式のみを出力すること）
  {
    "question": "タイ語の穴埋め問題文 [発音記号] (日本語訳)",
    "choices": [
      { "text": "タイ語 (発音記号)", "isCorrect": true },
      { "text": "タイ語 (発音記号)", "isCorrect": false },
      { "text": "タイ語 (発音記号)", "isCorrect": false }
    ],
    "explanation": "優しくわかりやすい解説。（※A,Bなどの記号は使わず、単語そのものを引用すること）\\nで改行を入れ、最後に励ましの言葉を入れてください。"
  }`;
